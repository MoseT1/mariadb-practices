...
select period_diff( date_format(now(), '%Y%m'),date_format(max(birth_date), '%Y%m'))/12 as '최연소 나이', period_diff( date_format(now(), '%Y%m'),date_format(min(birth_date), '%Y%m'))/12 as '최고령 나이'
from employees;
